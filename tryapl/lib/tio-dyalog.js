...
var tioParams     = ["Vlang","1","apl-dyalog","Vargs","0","F.code.tio","0","F.input.tio"];
var runRequest;
  ...
  document.addEventListener("keydown", function(event) {     
    ...
    if (!shiftd && !ctrld && !session.disabled && event.keyCode === 13) {  
      if (!getCurrentLine().replace(/\s/g, '').length) {
        ...
        submitLine(getCurrentLine(), tioParams);                      
      }
...
function submitLine(currentLine, params) {
  ...
  return tioProcess(runURL, currentLine, lastRequest, params); 
}

async function tioProcess(runURL, currentLine, code, params) {  
